...
let arr3 = ["박현지", 1, 2, 3, null, { name: "hyeonji", age: 23 }];
console.log(arr3);
...
console.log("arr4.concat(arr5):", concatArr);
...
// 2-10 filter 필터는 배열 요소 전체를 대상으로 조건을 걸어서 그 조건을 충족하는 결과를 새로운 배열을 반환해준다.
...
// 2-11 map  map은 배열 요소 전체를 대상으로 함수를 호출하고, 그 결과를 새로운 배열을 반환할때 주로 사용한다.
let countries1 = ["Österreich", "Andorra", "Vietnam", "Korea", "China"];
let countriesLengths = countries1.map((item) => item.length);
console.log("countriesLengths: ", countriesLengths);

// 2-12 reduce map은 배열을 반환할때 사용했지만 reduce는 값 하나를 반환할때 주로 사용 대표적인 예시로 1 ~ n 까지 더하기가 있다.
